(function () {
  ...
  var LS_KEY = "geongikbus_selection_v1";
  var LS_HOLIDAY_KEY = "geongikbus_force_holiday_v1";
  ...
  function renderRouteList() {
    ...
    sortedRoutes().forEach(function (route) {
      ...
      var jointBadge = route.joint ? '<div class="rc-joint">공동배차: ' + route.companies.join("·") + ' (경익버스 담당 차량만 표시)</div>' : "";
      ...
      btn.innerHTML =
        ...
        '<div class="rc-path">' + route.origin + ' ↔ ' + route.destination + ' · 경익버스 배차 ' + trips.length + '대</div>' +
  ...
  function renderTripList() {
    ...
    if (trips.length === 0) {
      container.innerHTML = '<p>오늘(' + dayCategoryLabel(state.dayCategory) + ') 경익버스 배차 차량이 없습니다.</p>';
      return;
    }
  ...
  // via = 그 회차에만 지원운행으로 들르는 곳. 출발지와는 다른 개념이다.
  function viaPoint(stop) {
    return (stop.via && stop.via !== "") ? stop.via : null;
  }
  ...
  function renderScheduleList() {
    ...
    state.trip.stops.forEach(function (s, idx) {
      ...
      var viaHtml = viaPoint(s) ? '<span class="sl-via">' + viaPoint(s) + ' 경유</span>' : "";
      var offHtml = (idx > limitIdx) ? '<span class="sl-noalarm">알림 없음</span>' : "";
      if (isMidStart(s)) li.classList.add("mid-start");
      li.innerHTML =
        '<span class="sl-stop">' + departPoint(s) + ' 출발' + midHtml + viaHtml + offHtml + '</span>' +
  ...
  function buildICS() {
    ...
    trip.stops.forEach(function (s, idx) {
      ...
      var desc = route.origin + " ↔ " + route.destination + " / " +
        ...
        (viaPoint(s) ? " / 이 회차만 " + viaPoint(s) + " 경유(지원운행)" : "");
      lines.push("BEGIN:VEVENT");
      lines.push("UID:geongikbus-" + todayStr(now) + "-" + route.label.replace(/\s/g, "") + "-" + trip.trip + "-" + idx + "@geongikbus");
  ...
  function exportICS() {
    ...
    a.href = url;
    a.download = "geongikbus-" + todayStr(new Date()) + ".ics";
  ...
  var LS_SOUND_KEY = "geongikbus_sound_v1";
  ...
  function fireAlarm(stop, leadMin) {
    ...
    if (window.Notification && Notification.permission === "granted") {
      try {
        new Notification("경익버스 배차 알림 - " + leadMin + "분 전", {
          body: where,
          icon: "icon-192.png"
        });
      } catch (e) { /* ignore */ }
